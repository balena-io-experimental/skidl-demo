                    ...
                    if state == 'in-component':
                        ...
                        if (component_ref is not None and
                            ...
                            self.components[component_id] = (
                                component_ref, component_pos)
                            self.extend_range(
                                component_pos[0], component_pos[1])
                            component_ref = None
                            component_id = None
                            component_pos = None
...
def move_modules(components, board, offsets):
    for module in board.GetModules():
        ...
        if path in components:
            ...
            new_pos = pcbnew.wxPoint(
                pos[0] * POS_SCALE, (pos[1] + offset) * POS_SCALE)
...
class SchematicPositionsToLayoutPlugin(pcbnew.ActionPlugin):
    ...
    def Run(self):
        ...
        DEBUG = open(os.path.join(
            work_dir, 'schematic-positions-to-layout.debug'), 'w')
        try:
            self.DoRun()
        finally:
            DEBUG.close()
    ...
    def DoRun(self):
        ...
        print('root_schematic_file = {}'.format(
            root_schematic_file), file=DEBUG)
        ...
        while len(sheet_queue) > 0:
            ...
            if sheet_path in sheets:
                print('Oops. Sheet "{}" turned up twice!'.format(
                    sheet_path), file=DEBUG)
                sys.exit(1)
            ...
            for sub_sheet_name in sheet.sub_sheets:
                sheet_queue[sheet_path + '/' +
                            sub_sheet_name] = sheet.sub_sheets[sub_sheet_name]
        ...
        offsets[''] = 0
        OFFSET_FACTOR = 1.25
        running_offset = OFFSET_FACTOR * \
            (sheets[''].yrange[1] - sheets[''].yrange[0])
        for sname in sheets:
            if sname == '':
                continue
            s = sheets[sname]
        ...
        for sheet_name in sheets:
            ...
            for cid in s.components:
                components[sheet_name + '/' +
                           cid] = s.components[cid] + tuple([sheet_name])
